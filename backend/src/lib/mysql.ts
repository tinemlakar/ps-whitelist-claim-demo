...
export class MySql {
    ...
    if (!this.db) {
      try {
        this.db = await mysql.createPool({
          ...
          password:
            this.env.APP_ENV === 'testing' ? this.env.MYSQL_PASSWORD_TEST : this.env.MYSQL_PASSWORD,
          database: this.env.APP_ENV === 'testing' ? this.env.MYSQL_DB_TEST : this.env.MYSQL_DB,
          waitForConnections: true,
          decimalNumbers: true,

          connectionLimit:
            (this.env.APP_ENV === 'testing' ? this.env.MYSQL_POOL_TEST : this.env.MYSQL_POOL) || 10,
        ...
        const host =
          this.env.APP_ENV === 'testing' ? this.env.MYSQL_HOST_TEST : this.env.MYSQL_HOST;
        const port =
          this.env.APP_ENV === 'testing' ? this.env.MYSQL_PORT_TEST : this.env.MYSQL_PORT;
        const database =
          this.env.APP_ENV === 'testing' ? this.env.MYSQL_DB_TEST : this.env.MYSQL_DB;
        writeLog(
          LogType.INFO,
  ...
  public async paramExecuteBatch(query: string, values?: Object[], connection?: PoolConnection) {
    ...
    const sqlParamValuesLog =
      omitLong && query.length > 3000 ? 'query params omitted (too long)' : sqlParamValues;
